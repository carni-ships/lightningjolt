...
pub fn init_cache(g1_vec: &[ArkG1], g2_vec: &[ArkG2]) {
    ...
    // Prepare the new cache — G1 and G2 are independent, prepare in parallel
        ...
        rayon::join(
            || {
                g1_vec
                    ...
                    .collect::<Vec<<Bn254 as Pairing>::G1Prepared>>()
            },
            || {
                g2_vec
                    .par_iter()
                    ...
                    .collect::<Vec<<Bn254 as Pairing>::G2Prepared>>()
            },
        )
    };
    #[cfg(not(feature = "parallel"))]
    let (g1_prepared, g2_prepared) = {
        let g1: Vec<<Bn254 as Pairing>::G1Prepared> = g1_vec
            .iter()
            .map(|g| {
                let affine: G1Affine = g.0.into();
                affine.into()
            ...
            .collect();
        let g2: Vec<<Bn254 as Pairing>::G2Prepared> = g2_vec
            .iter()
            .map(|g| {
                let affine: G2Affine = g.0.into();
                affine.into()
